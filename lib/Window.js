...
module.exports = (Item) => {
  return class Window extends EventEmitter {
    ...
    acceptClick (click) {
      assert.ok(click.mouseButton === 0 || click.mouseButton === 1 || click.mouseButton === 2)
      if (click.slot === -999) {
    ...
    acceptNonInventorySwapAreaClick (click) {
      // Enable middle click
      assert.ok(click.mode === 0 || click.mode === 3)
      if (click.mouseButton === 0 && click.mode === 0) {
        this.acceptSwapAreaLeftClick(click)
      } else if (click.mouseButton === 1 && click.mode === 0) {
        this.acceptSwapAreaRightClick(click)
      // Enable middle click
      } else if (click.mouseButton === 2 && click.mode === 3) {
      } else {
        assert.ok(false, 'unimplemented')
      }
    }
