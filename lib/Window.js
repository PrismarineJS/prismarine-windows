...
module.exports = (mcData, Item) => {
  return class Window extends EventEmitter {
    ...
    acceptClick (click) {
      ...
      // if (mcData.supportFeature('clientSideAnvilInventoryManagement') && click.inAnvil && click.slot >= 1 && click.slot <= 2) {
      if (mcData.isNewerOrEqualTo('1.17') && click.inAnvil && click.slot >= 1 && click.slot <= 2) {
        return this.acceptInAnvilCombineClick(click)
      } else if (click.slot === -999) {
    ...
    acceptInAnvilCombineClick (click) {
      let changedSlots
      switch (click.slot) {
        case 0:
          if (click.updateAnvilManually) {
            this.updateSlot(0, click.selectedItem)
            click.selectedItem = null
          }
          changedSlots = [
            {location: 0, item: Item.toNotch(this.slots[0])}
          ]
          break
        case 1:
          if (click.updateAnvilManually) {
            this.updateSlot(1, this.selectedItem)
            this.updateSlot(2, click.expectedNewItem)
            this.selectedItem = null
          }
          changedSlots = [
            {location: 2, item: Item.toNotch(click.expectedNewItem)},
            {location: 1, item: Item.toNotch(this.slots[1])}
          ]
          break
        case 2:
          let changedSlotOne = false
          if (click.updateAnvilManually) {
            this.updateSlot(0, null)
            if (this.slots[1] !== null) {
              changedSlotOne = true
              this.updateSlot(1, null)
            }
            this.selectedItem = click.expectedNewItem
            this.updateSlot(2, null)
          }
          changedSlots = [
            {location: 0, item: Item.toNotch(null)},
            {location: 2, item: Item.toNotch(null)}
          ]
          if (changedSlotOne) {
            changedSlots.push({location: 1, item: Item.toNotch(null)})
          }
          break
      }
      return changedSlots
