...
module.exports = (Item) => {
  return class Window extends EventEmitter {
    ...
    acceptClick (click) {
      assert.ok((click.slot >= 0 && click.slot < this.inventoryEnd || click.slot === -999) && click.mouseButton >= 0 && click.mouseButton <= 6, 'invalid operation')
    ...
    shiftClick (click) {
      ...
      if (this.type === 'minecraft:inventory') {
        if (click.slot > this.hotbarStart) {
          this.fillAndDump(item, this.inventoryStart, this.inventoryEnd - 10)
        } else if (click.slot >= this.inventoryStart && click.slot < this.inventoryEnd - 10) {
    ...
    numberClick (click) {
      ...
      if (item) {
        if (itemAtHotbarSlot) {
          ...
          // idk how to use mcData here, pls implement somebody
    ...
    fillAndDump (item, start, end, lastToFirst = false) {
      this.fillSlotsWithItem(this.findItemsRange(start, end, item.type, item.metadata, true, item.nbt), item, lastToFirst)
