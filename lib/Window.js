    ...
    clear (blockId = null, count) {
      ...
      if (!count && !blockId) {
        ...
        for (let i = this.hotbarStart + 9 + 1; i > this.hotbarStart - 1; i--) {
          const currSlot = this.slots[i]
          const blocksNeeded = count - currCount
          if (currSlot?.type === blockId) {
            if (currSlot.count > blocksNeeded) { // stack is bigger then needed
              partial.item = new Item(blockId, currSlot.count - blocksNeeded, currSlot.metadata, currSlot.nbt)
              partial.used = true
              partial.slot = i
        ...
        if (partial.used) {
          this.updateSlot(partial.slot, partial.item)
        }
